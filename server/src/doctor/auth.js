...
const register = async (name, email, password ) => {
    try {
      ...
      if (existingUser) return { message: "Doctor already exists", status: 403 };
      ...
      return { message: "Error registering doctor", status: 500 };
    }
  };
...
const login = async (email, password) => {
  try {
    ...
    if (!user) return { message: "Doctor not found", status: 404 };
